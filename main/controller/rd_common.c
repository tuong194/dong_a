...
static Countdown_t countdown_val = {0};
static flash_config_value_t flash_config_value = {0};
static esp_timer_handle_t countdown_timer_handle = NULL;
...
int init_flash_config(void)
{
    ...
    if ((flash_config_value.header[0] != FLASH_HEADER_1 && flash_config_value.header[1] != FLASH_HEADER_2 &&
        ...
        !check_add_home())
    {
        ESP_LOGW("COMMON", "init flash default");
...
int loop_check_save_flash(void)
{
    if (flash_check_save.is_save_flash)
    {
        if (rd_exceed_us(flash_check_save.time_start_save_flash, TIMEOUT_SAVE_FLASH_CONFIG))
        {
            ...
            for (uint8_t i = 0; i < NUM_ELEMENT; i++)
            {
                if (flash_config_value.stt_startup == START_RESTORE)
...
static void countdown_cb(void *args)
{
    countdown_val.time_now_s++;
    if (countdown_val.time_now_s >= countdown_val.time_countdown_s)
    {
        ...
        tb_publish_telemetry("{\"CompleteCountdown\":0}");
        rd_stop_countdown();
    }
}

static void rd_config_countdown(void)
{
    if (!countdown_timer_handle)
    {
        esp_timer_create_args_t countdown_timer = {0};
        countdown_timer.arg = NULL;
        countdown_timer.callback = countdown_cb;
        countdown_timer.dispatch_method = ESP_TIMER_TASK;
        countdown_timer.name = "countdown_timer";
        esp_timer_create(&countdown_timer, &countdown_timer_handle);
    }
}
esp_err_t rd_start_countdown(int64_t time_s, stt_target_coundown target)
{
    ESP_LOGI("COMMON", "start countdown timer");
    countdown_val.stt_target = target;
    countdown_val.time_countdown_s = time_s;
    countdown_val.time_now_s = 0;
    return esp_timer_start_periodic(countdown_timer_handle, 1 * 1000U * 1000U);
}

esp_err_t rd_stop_countdown(void)
{
    ESP_LOGE("COMMON", "stop countdown timer");
    return esp_timer_stop(countdown_timer_handle);
}
...
void rd_set_status_startup(stt_startup_t stt_startup)
{
    ...
    {
    case START_OFF:
        flash_config_value.stt_startup = START_OFF;
        break;
    case START_ON:
        flash_config_value.stt_startup = START_ON;
        break;
    case START_RESTORE:
        flash_config_value.stt_startup = START_RESTORE;
        break;
